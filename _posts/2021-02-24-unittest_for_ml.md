...
#### 1. 검증 대상을 기준으로 Test가 이뤄지면 안 된다.

Test Code를 작성할 때에는 검증 대상에 속하는 코드를 사용하지 않아야 합니다. 예를 들어 어떤 Class를 구성하는 각 Method에 대한 Test Code를 작성하고 있다면 해당 Class의 다른 Method와 Attribute 또한 검증 대상이라고 할 수 있으므로 사용하지 않는 것이 좋습니다. 대신 Hard Coding을 통해 매번 적절한 값이 나오는지 비교할 수 있도록 하는 것을 추천합니다.
